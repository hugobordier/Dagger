...
public class UIManager : MonoBehaviour
{
    ...
    [SerializeField] private int m_TotalHeartSlots = 3; 

    [Header("Visuels")]
    [SerializeField] private Sprite m_FullHeartSprite;  // Glisse ton image coeur rouge ici
    [SerializeField] private Sprite m_EmptyHeartSprite; // Glisse ton image coeur vide/gris ici

    private List<Image> m_HeartImages = new List<Image>(); // On stocke directement les composants Image

    void Start()
    {
        if (m_Player == null || m_HeartPrefab == null) return;
